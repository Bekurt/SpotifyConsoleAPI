    ...
    try
        ...
        if String.IsNullOrWhiteSpace(clientId) || String.IsNullOrWhiteSpace(clientSecret) then
            ...
            | "auth" :: _ ->
                ...
                Auth.authorizeAsync clientId clientSecret redirectUri scopes |> Async.AwaitTask |> Async.RunSynchronously
                0
            | "search" :: rest when rest.Length > 0 ->
                let query = String.Join(" ", rest)
                let token = Auth.getAccessToken clientId clientSecret "http://localhost:5000/callback"
                let tracks = Search.searchTracksAsync token query |> Async.AwaitTask |> Async.RunSynchronously
